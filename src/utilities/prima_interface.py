...
from src.utilities.prima_util import KAct, encode_kactivation_cons


def get_prima_constraints(
    input_lb: Tensor,
    input_ub: Tensor,
    prima_hyperparameters: Dict[str, float],
    ...
    :returns:
        ...
        const_coefs
    """
    input_lb, input_ub = _refine_bounds_for_candidate_unstable_neurons(
        input_lb, input_ub, batch_size, layer_shape, intermediate_bounds_callback
    )
    ...
    batch_prima_constraints = encode_kactivation_cons(
        ...
        max_number_of_parallel_input_constraint_queries=prima_hyperparameters[
            "max_number_of_parallel_input_constraint_queries"
        ],
        max_unstable_nodes_considered_per_layer=prima_hyperparameters[
            "max_unstable_nodes_considered_per_layer"
        ],
        min_relu_transformer_area_to_be_considered=prima_hyperparameters[
            "min_relu_transformer_area_to_be_considered"
        ],
        fraction_of_constraints_to_keep=prima_hyperparameters[
...
def _refine_bounds_for_candidate_unstable_neurons(
    ...
    refined_lb[candidate_unstable_neuron_indices] = np.maximum(
        lb[candidate_unstable_neuron_indices], lb_with_other_params
    )
    refined_ub[candidate_unstable_neuron_indices] = np.minimum(
        ub[candidate_unstable_neuron_indices], ub_with_other_params
    )

    return np.asarray(refined_lb), np.asarray(refined_ub)
...
def _build_sparse_prima_coefficient_matrix(
    ...
    for batch_index, prima_constraints in enumerate(batch_prima_constraints):
        n_prima_coefficients_in_batch_element = sum(
            len(constraint_group.varsid) * constraint_group.cons.shape[0]
            for constraint_group in prima_constraints
        )
        batch_indices += [
        ...
        for constraint_group in prima_constraints:
            ...
            const_coefs_of_group = torch.tensor(
                constraint_group.cons[:, 0], dtype=torch.float32
            )
            const_coefs[
    ...
    output_coefs = torch.sparse_coo_tensor(
        full_indices_of_non_zero_elements, output_coef_values, size=dense_coefs_shape
    )
    input_coefs = torch.sparse_coo_tensor(
        full_indices_of_non_zero_elements, input_coef_values, size=dense_coefs_shape
    )
